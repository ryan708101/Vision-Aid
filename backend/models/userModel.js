...
const userSchema = new mongoose.Schema({
    ...
    detectedDisease: { type: String, default: "Normal" },
    
    // Current progress (4 weeks instead of 7)
    curWeek: { type: Number, default: 1 },
    curChallenge: { type: Number, default: 1 },
    selectedWeek: { type: Number, default: 1 },
    
    // Badges for 4 weeks
    badges: { type: [Boolean], default: [false, false, false, false] },
    
    // Activity tracking
    activityDates: [
        {
            date: { type: Date, required: true },
            challengeNo: { type: Number, required: true },
            weekNo: { type: Number, required: true }
        }
    ],
    
    // Scores: 4 weeks x 7 challenges per week
    scores: {
        type: [[Number]],
        default: Array.from({ length: 4 }, () => Array(7).fill(0.0)),
    },
    
    // Last played date for "one game per day" rule
